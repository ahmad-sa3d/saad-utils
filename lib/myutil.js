...
// {
	...
	{
		myUtil.documentElement = {};
		myUtil.isStandardMode = true;
	}
	...
	/**
	 * Load Javaascript File
	 * @param  {string} filename file path
	 */
	myUtil.loadJsFile = function( filePath )
	{
	 ...
	 */
	myUtil.loadCssFile = function( filePath )
	{
	 ...
	 */
	myUtil.trimText = function( text, from, length, tail ){
		var trimmedText = text.substr( from, length );

		if( typeof tail != 'undefined' )
		{
			if( trimmedText.length < text.length )
				trimmedText = trimmedText + ' ' + tail;
		}
	 ...
	 * @return {[type]}      [description]
	 */
	myUtil.reverseDate = function( date, delimiter ){
		...
		var dateArray= text.split( date, delimiter );

		var reversed = dateArray.reverse().join( delimiter );
		return reversed;
	}
	 ...
	 * @param  {string} filename file path
	 */
	myUtil.getUrlParameters = function()
	{

        if( typeof requestParams == 'undefined' )
        {
			// Set Request Parameters
			var string = decodeURIComponent( window.location.search.replace( /^\?/, '' ) );
	 ...
	 * @param  {string} filename file path
	 */
	myUtil.getUrlParameter = function( name, defaultValue )
	{
		myUtil.getUrlParameters();

        if( typeof requestParams[ name ] == 'undefined' || requestParams[ name ]  == null )
        	return defaultValue;
        else
	 ...
	 * @return {[type]}         [description]
	 */
	myUtil.buildQueryString = function( dataObj ) {
		var string = '';
		...
		for( p in dataObj )
		{
			...
			string += encodeURIComponent( p ) + '=' + encodeURIComponent( dataObj[ p ] );
		}
	 ...
	 * @return {[type]}         [description]
	 */
	myUtil.updateBrowserQueryString = function( dataObj ) {

		if( window.history && typeof window.history.pushState == 'function' )
			window.history.pushState( null, null, window.location.pathname + myUtil.buildQueryString( dataObj ) );
	}
	 ...
	 * @param  {string} filename file path
	 */
	myUtil.getFormattedDate = function( day, month, year, locale, withYear )
	{
		if( typeof day == 'undefined' || typeof month == 'undefined' || typeof year == 'undefined' )
			console.error( 'myUtil.getFormattedDate requires day, month, year and optional locale' );
		...
		if( typeof withYear != 'undefined' && ! withYear )
			year = '';
		else
			year = ', ' + year;

		if( typeof locale != 'undefined' && locale == 'ar' )
		{
	 ...
	 * @param  {string} filename file path
	 */
	myUtil.getDayName = function( day, locale, isAppreviate )
	{
		if( typeof day == 'undefined' )
			console.error( 'myUtil.getDayName requires day, and optional locale, isAppreviate' );

		if( day < 0 || day > 6 )
			console.error( 'myUtil.getDayName day must be in range of 0 and 6' );
		...
		if( typeof locale != 'undefined' && locale == 'ar' )
		{
			return ( typeof isAppreviate != 'undefined' && isAppreviate ) ? days_ar_appr[ day ] : days_ar[ day ];
		}
		else
		{
			return ( typeof isAppreviate != 'undefined' && isAppreviate ) ? days_en_appr[ day ] : days_en[ day ];
		}
...
// })( window.myUtil || {});
	...
	*								Routes
	*---------------------------------------------------------------------------*/
	myUtil.urlToRoute = function( name, params )
	{
		//
